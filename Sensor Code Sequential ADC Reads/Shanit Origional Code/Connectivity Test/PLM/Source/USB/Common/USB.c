...
void USB_Init(void)
{
  ...
  /* Create the USB task */
  gUsbLockLevel = 0;
//  gUsbTaskId = TS_CreateTask(gTsUsbTaskPriority_c, USB_Task); //@SMAC
...
//    TS_SendEvent(gUsbTaskId, gUsbIrqEvent_c);  //@SMAC
    USB_Task();
...
* 
************************************************************************************/
static void USB_Task(void)  //@SMAC
{ 
  usbMsg_t* pUsbMsg;
