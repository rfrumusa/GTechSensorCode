...
static uint16_t gaDeviceStateBeforeSuspend[(((1)?1:0) + ((0)?1:0))];
...
)
{
  ...
  (void)USB_DeviceGetStatus(event->controllerId,(0x01), &gaDeviceStateBeforeSuspend[event->controllerId]);
   
  (void)USB_DeviceSetStatus(event->controllerId, (0x01),(0x80));
}
...
)
{
  ...
  if(deviceState == (0x80))
  {
    ...
    (void)USB_DeviceSetStatus(event->controllerId,
            (0x01), gaDeviceStateBeforeSuspend[event->controllerId]);
  }
}
...
)
{
  ...
  for(i=0 ; i< (((1)?1:0) + ((0)?1:0)) ; i++)
  {
    (void)USB_DeviceGetStatus( i , (0x01), &deviceState);
    if(deviceState < (0x80))
    {
      (void)USB_DeviceSetStatus( i , (0x05) , sofCount);   
    }
  }
  return;
}
...
)
{
  ...
  epStruct.direction = gUsbEpDirection_In_c;	
  (void)USB_DeviceInitEndpoint(event->controllerId, &epStruct, 1);
  epStruct.direction = gUsbEpDirection_Out_c;
  (void)USB_DeviceInitEndpoint(event->controllerId, &epStruct, 1);
   
  (void)USB_DeviceSetStatus(event->controllerId , (0x01) , (0x02));
   
  (void)USB_DeviceSetStatus(event->controllerId, (0x06), (0x40) >> (6));
  ...
  gpfClassFrameworkCallback(event->controllerId, gUsbToAppEvent_BusReset_c, &usbMsg);
  return;
}
...
)
{
  ...
  gpfClassFrameworkCallback(event->controllerId, gUsbToAppEvent_Error_c,&usbMsg);
  return;
}
...
uint8_t USB_ClassInit (
    uint8_t    controllerId,                 
    pfUsbToAppEventHandler_t classCallback,       
    pfusbReqHandler_t       otherReqCallback    
)
{
  uint8_t status = USB_Framework_Init(controllerId , classCallback , otherReqCallback);
   
  gpfClassFrameworkCallback = classCallback;
  if(status == (0x00))
  {
     
    status |= USB_DeviceRegisterService(controllerId , ((16 - 1) + 1) , USB_Service_Reset);
    status |= USB_DeviceRegisterService(controllerId , (((16 - 1) + 1) + 1) , USB_Service_SOF);
    status |= USB_DeviceRegisterService(controllerId , (((((16 - 1) + 1) + 1) + 1) + 1) , USB_Service_Suspend);
    status |= USB_DeviceRegisterService(controllerId , ((((16 - 1) + 1) + 1) + 1),USB_Service_Resume);
    status |= USB_DeviceRegisterService(controllerId , (((((((16 - 1) + 1) + 1) + 1) + 1) + 1) + 1) , USB_Service_Stall);
    status |= USB_DeviceRegisterService(controllerId , ((((((16 - 1) + 1) + 1) + 1) + 1) + 1) , USB_Service_Error);
      
    (void)USB_DeviceSetStatus(controllerId , (0x01) , (0x03));
    gaDeviceStateBeforeSuspend[controllerId] = (0x03);
  }
  return status;
}
...
)
{
  ...
  ccr = IntDisableAll();
  (void)USB_DeviceGetStatus(controllerId , (0x01) , &deviceState);
  (void)USB_DeviceGetStatus(controllerId , (0x06) , &state);
  if((deviceState == (0x80)) && 
           (state & (0x0002) ) && 
           (USB_Desc_RemoteWakeup(controllerId) == 1))
  {
     
      USB_DeviceAssertResume(controllerId);    
      deviceState = gaDeviceStateBeforeSuspend[controllerId];
     
      (void)USB_DeviceSetStatus(controllerId,(0x01), gaDeviceStateBeforeSuspend[controllerId]);
