...
static usbServiceCallback_t gUsbCB_[((((((((16 - 1) + 1) + 1) + 1) + 1) + 1) + 1) + 1)];  
 
 
static uint16_t gUsbComponentStatus_[(0x07)]; 
 
static uint16_t gUsbEpStatus_[16];
...
static void USB_DeviceInitParams(uint8_t    controllerId)
{
  ...
  for(loopIndex = 0; loopIndex < (0x07); loopIndex++)
  {
    gUsbComponentStatus_[loopIndex] = (0xFFFF);
  }
   
  for(loopIndex = 0; loopIndex < 16; loopIndex++)
  {
    gUsbEpStatus_[loopIndex] = (5);
  }
}
...
)
{
  ...
  if((endpoints > 16) || (endpoints < 1))
  {
    return (0x99);
  }
   
  gEpNoMax_ = endpoints - 1;
  USB_DeviceInitParams(controllerId);
   
  MemorySet(gUsbCB_, 0, (uint16_t)(sizeof(usbServiceCallback_t) * ((((((((16 - 1) + 1) + 1) + 1) + 1) + 1) + 1) + 1)));
...
)
{
  ...
  if((gEpNo_ == 0) && (pUsbEpStruct->number != (0)))
  {
    return (0x95);
  }
  ...
  if ((pUsbEpStruct->number != (0)) && (status == (0x00)))
  {
    gEpNo_--;
  }
...
)
{
  ...
  uint8_t epNum = (uint8_t)(component & (0x0F));
  (void)controllerId;
  
  if((component <= (0x07)) && (component >= (0x01)))
  {
   
    *status = gUsbComponentStatus_[component-1];
  }
  else if ((component & (0x10)) && (epNum < 16))
  {
     
    *status = gUsbEpStatus_[epNum];
  }
  else
  {
    return (0x82);
...
)
{
  ...
  if((component <= (0x07)) && (component >= (0x01)))
  {
    ...
    gUsbComponentStatus_[component-1] = setting;
  }
  else if ((component & (0x10)) && (epNum < 16))
  {
    uint8_t direction = (uint8_t)((component >> (7)) & (0x01));
     
    if(setting == (1))
    {
      USB_DeviceStallEndpoint(controllerId, epNum, direction);
    }
    else if((setting == (0)) && (gUsbEpStatus_[epNum] == (1)))
    {
...
)
{
  ...
  if(((type  <= (16 - 1)) ||
        ((type < ((((((((16 - 1) + 1) + 1) + 1) + 1) + 1) + 1) + 1)) && (type >= ((16 - 1) + 1)))) &&
       (gUsbCB_[type] == (( void * )( 0 ))))
  {
     
    gUsbCB_[type] = service;
    return (0x00);
  }
  else
  {
...
)
{
  ...
  if(gUsbCB_[type] != (( void * )( 0 )))
  {
     
    gUsbCB_[type](pUsbDeviceEvent);
  }
