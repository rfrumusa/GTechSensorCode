...
uint8_t gDTEStatus = (uint8_t)(0xffffffff);  
static uint8_t gCurrentInterface = 0;
static uartBitmap_t gUARTBitmap;

static uint8_t gSerialState[(0x08)+(0x02)] =
{
  (0xA1),(0x20),
  0x00,0x00, 
  0x00,0x00, 
...
)
{
  ...
  *pSize = 0;
  ...
  gCurrentInterface = (uint8_t)pSetupPacket->index ;
  gUARTBitmap.byte = 0x00;  
   
  gUARTBitmap.bitmapUart.bTxCarrier = (gDTEStatus & (0x02)) ? 1 : 0 ;
  ...
  if(pstnCallback != (( void * )( 0 )))
  {
    usbMsg_t usbMsg;
    usbMsg.controllerId = controllerId;
    if((gDTEStatus & ((0x01))) == ((0x01)))
    {
      usbMsg.eventType = (gUsbToAppEvent_t)(0x25);       
    }
    else
    {
      usbMsg.eventType = (gUsbToAppEvent_t)(0x26);    
    }
    USB_IsrEvent(&usbMsg);  
  }
  return (0x00);
...
)
{
  ...
  gSerialState[(0x08)+(0x02)-2] = gUARTBitmap.byte;
  if(gNotifyEndpoint)
  {
    (void)USB_DeviceCancelTransfer (controllerId, gNotifyEndpoint, gUsbEpDirection_In_c);
    (void)USB_ClassSendData_( controllerId , gNotifyEndpoint , gSerialState , ((0x08) + (0x02)) , 0);    
  }
