...
static uint32_t mClkSourceKhz = (uint32_t)(48000);
...
{
    return (milliseconds * (mClkSourceKhz/(0x01<<0x07)));
} 
...
)
{
    ...
    NVIC_SetPriority(42, 3);

     
    ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= (1<<24);

                            
    (void)(((((FTM_MemMapPtr)0x40038000u))->SC) == 0U);
     
    ((((FTM_MemMapPtr)0x40038000u))->SC) = (uint32_t)0x00UL;
     
    (void)(((((FTM_MemMapPtr)0x40038000u))->CONTROLS[0]. CnSC) == 0U);             

     
     
    ((((FTM_MemMapPtr)0x40038000u))->MODE) |= (uint32_t)0x04UL;       
     
    ((((FTM_MemMapPtr)0x40038000u))->MODE) &= (uint32_t)~0x01UL;     

     
    ((((FTM_MemMapPtr)0x40038000u))->MOD) = (uint32_t)0x00UL;	

    ((((FTM_MemMapPtr)0x40038000u))->CONTROLS[0]. CnSC) = (uint32_t)0x50UL; 
...
) 
{
    ...
    if(((((FTM_MemMapPtr)0x40038000u))->SC) & 0x80) 
    {
        ((((FTM_MemMapPtr)0x40038000u))->SC)  &= ~ 0x80;
    }

    if ( ((((FTM_MemMapPtr)0x40038000u))->CONTROLS[0]. CnSC) & 0x80 ) 
    {
        ((((FTM_MemMapPtr)0x40038000u))->CONTROLS[0]. CnSC) &= ~0x80;
        TMR_Task();
    }
}
...
)
{
    ...
    if (!(maTmrTimerStatusTable[(timerID)]) || (0xFF == timerID)) 
      return;
...
)
{
    ...
    (currentTimeInTicks) = ((((FTM_MemMapPtr)0x40038000u))->CNT);;
	...
	(ticksdiff) = ((((FTM_MemMapPtr)0x40038000u))->CNT);;  
    ...
    ((((FTM_MemMapPtr)0x40038000u))->CONTROLS[0]. CnV) = nextInterruptTime;
    IntRestoreAll(saveInt);

	if (!numberOfActiveTimers && !numberOfLowPowerActiveTimers) 
    {
		(((((FTM_MemMapPtr)0x40038000u))->SC) = 0x00);
		timerHardwareIsRunning = 0;
	} 
    else if (!timerHardwareIsRunning) 
    {
		(((((FTM_MemMapPtr)0x40038000u))->SC) |= (0x08 | 0x07));
...
)
{
    ...
    (((((FTM_MemMapPtr)0x40038000u))->SC) |= (0x08 | 0x07));
    (previousTimeInTicks) = ((((FTM_MemMapPtr)0x40038000u))->CNT);;
