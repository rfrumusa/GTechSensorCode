...
{
  if(pfCDCDicInServiceCallBack != (( void * )( 0 )))
  {
...
{   
  if(pfCDCDicOutServiceCallBack != (( void * )( 0 )))
  {
...
)
{
  ...
  if(event == gUsbToAppEvent_EnumComplete_c)
  {
    ...
    while(index < count)
    {
      ...
      (void)USB_DeviceInitEndpoint(controllerId, epStruct, 1);
      ...
      switch(epStruct->type)
      {
        case gUsbEpType_Interrupt_c :
               ...
               (void)USB_DeviceRegisterService(controllerId,
                 (uint8_t)((0x00)+epStruct->number),USB_Class_CDC_ServiceCicNotify);
        break;
        case gUsbEpType_Bulk_c :
               if(epStruct->direction == gUsbEpDirection_Out_c)
               {
                 (void)USB_DeviceRegisterService(controllerId,
                    (uint8_t)((0x00)+epStruct->number),USB_Class_CDC_ServiceDicBulkOut);
               }
               else
               {
                 (void)USB_DeviceRegisterService(controllerId,
                   (uint8_t)((0x00)+epStruct->number),USB_Class_CDC_ServiceDicBulkIn);
               }
        break;
      ...
      (void)USB_DeviceSetStatus(controllerId,
                (uint8_t)((0x10) | epStruct->number |
                (epStruct->direction << (7))),
                (uint8_t)(0));
      index++;
    }
  }
...
{
  uint8_t status = (0xCD);
  if((setupPacket->requestType & (0x60)) == (0x20))
  {  
