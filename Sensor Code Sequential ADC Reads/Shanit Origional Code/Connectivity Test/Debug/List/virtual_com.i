...
static cdcTxBufRef_t maCDCTxBufRefTable[3];
static index_t maCDCTxBufLenTable[3];
static index_t mCDCTxBufRefLeadingIndex;    
static index_t mCDCTxPendingTxNum;
static index_t mCDCTxBufRefTrailingIndex;   


static   uint8_t maCDCRxBuf[(16*4)];
static   index_t mCDCRxBufLeadingIndex;       
static   index_t mCDCRxBufTrailingIndex;      
volatile index_t gCDCRxBufferByteCount;       
...
{  
  ...
  if(USB_IsDeviceConnected(0) == 0)
  {
    ...
    gCDCDicRxMsgInUsbOueue = 0;
    gCDCDicTxMsgInUsbOueue = 0;
    gCDCDicRxEnabled = 1;
    ...
    usbErr =  USB_AddController(&usbClassDescriptor_CDC, 0);   
    if(usbErr == (0x00))
    {
      status = 1;   
    }
...
bool_t CDC_ModuleUninit(void)
{
   ...
   usbError_t usbErr;
   ccr = IntDisableAll();
   usbErr = USB_RemoveController(0);   
   if(usbErr == (0x00))
   {
     pfCDCRxCallBack = (( void * )( 0 ));
     USB_Class_CDC_SetDicInServiceCallback((( void * )( 0 )));
     USB_Class_CDC_SetDicOutServiceCallback((( void * )( 0 )));
     mCDCEnumerated = 0;
     mCDCChannelOpen = 0;
     status = 1;   
   }
...
bool_t CDC_Transmit(unsigned char const *pBuf, index_t bufLen, void (*pfCallBack)(unsigned char const *pBuf))
{
  ...
  void (*pfUncalledCallBack)(unsigned char const*) = (( void * )( 0 ));
  ...
  if(maCDCTxBufLenTable[mCDCTxBufRefLeadingIndex] == 0)
  {
    ...
    if (++mCDCTxBufRefLeadingIndex >= ((sizeof(maCDCTxBufRefTable) / (sizeof(maCDCTxBufRefTable[0]))))) 
    {
  ...
  if(transmit)
  {
    (void)USB_ClassSendData_( 0 , (1) , (uint8_t*)pBuf , bufLen , 0);    
  }
  if(pfUncalledCallBack != (( void * )( 0 )))
  {
...
void CDC_SetUsbSuspendCallBack(void (*pfCallBack)(void))
{
  ...
  *pDst = maCDCRxBuf[mCDCRxBufTrailingIndex];
  ...
  if ((gCDCDicRxEnabled == 0) && (gCDCRxBufferByteCount <= 8)) 
  {
    gCDCDicRxEnabled = 1;
    (void)USB_DeviceRecvData(0, (2), (( void * )( 0 )), 0);;
  }
  IntRestoreAll(ccr);
  return 1;
 }
...
static void CDC_DicInServiceCallback(usbDeviceEvent_t* event)
{
  ...
  if (pfCallBack)
  {
    if(gCDCDicTxMsgInUsbOueue == 0)
    {
      ...
      USB_IsrEvent(&usbMsg);
      gCDCDicTxMsgInUsbOueue = 1;
    }
  }
  maCDCTxBufLenTable[mCDCTxBufRefTrailingIndex] = 0;
       
  if (++mCDCTxBufRefTrailingIndex >= ((sizeof(maCDCTxBufRefTable) / (sizeof(maCDCTxBufRefTable[0]))))) 
  {
  ...
  if (maCDCTxBufLenTable[mCDCTxBufRefTrailingIndex]) 
  {
     ...
     (void)USB_ClassSendData_( 0 , (1) , pBuf , bufLen , 0); 
  }
}
...
static void CDC_DicOutServiceCallback(usbDeviceEvent_t* event)
{
  ...
  if(pfCDCRxCallBack != (( void * )( 0 )))
  {
    ...
    ccr = IntDisableAll();
    if (sizeof(maCDCRxBuf) - gCDCRxBufferByteCount >= (16)) 
    {
      (void)USB_DeviceRecvData(event->controllerId, (2), (( void * )( 0 )), 0);;
    }
    else
    {
      gCDCDicRxEnabled = 0;
...
)
{
  ...
  switch(event_type)
  {
    ...
    case gUsbToAppEvent_SendComplete_c:
           if(pUsbMsg->eventParam.sendCompleteEvent.epNum == (1))
           {
               void (*pfCallBack)(unsigned char const *pBuf);    
               unsigned char const *pBuf;
               uint32_t ccr;
               ...
               do
               {
                 pfCallBack = (( void * )( 0 )); 
                 ccr = IntDisableAll();
                 if((maCDCTxBufLenTable[txBufRefTableIndex] == 0) && (maCDCTxBufRefTable[txBufRefTableIndex].pfCallBack != (( void * )( 0 ))))
                 {
                   pfCallBack = maCDCTxBufRefTable[txBufRefTableIndex].pfCallBack;
                   pBuf= maCDCTxBufRefTable[txBufRefTableIndex].pBuf;
                   maCDCTxBufRefTable[txBufRefTableIndex].pfCallBack = (( void * )( 0 ));
                 } 
                 IntRestoreAll(ccr);
                 if (++txBufRefTableIndex >= ((sizeof(maCDCTxBufRefTable) / (sizeof(maCDCTxBufRefTable[0]))))) 
                 {
